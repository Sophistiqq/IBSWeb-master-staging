...
import {
  ...
  traceMethodCalls,
  extractModelInfo,
  analyzeActionRelations,
} from "./utils/dotnet-parser.js";
    ...
    tools: [
      {
        name: "execute_sql",
        description:
          "Run SQL queries against the IBS database. Prompt on Write is enforced by the client.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description:
          "Search for a C# method and get its body plus all related DTO/Model definitions.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        name: "analyze_action",
        description:
          "Deep-dive into a controller action and show all its relations.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        name: "trace_workflow",
        description:
          "Recursively trace service and repository calls for a given code block or method.",
        inputSchema: {
          type: "object",
          properties: {
            methodName: { type: "string" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "check_build_status") {
      ...
      return {
        content: [
          {
            type: "text",
            text: Formatter.formatBuildStatus(
              buildResult.success,
              parsed.errors,
              parsed.warnings,
            ),
    ...
    if (name === "search_code_context") {
      ...
      if (filePath) {
        methodBody = await findMethodInFile(
        ...
        for (const dir of searchDirs) {
          const files = await glob(
            path.join(PROJECT_ROOT, dir, "**", "*.cs").replace(/\\/g, "/"),
          );
          ...
          if (props.length > 0) {
            definitions[type] =
              ...
              props
                .map(
                  (p) =>
                    `| ${p.name} | ${p.type} | ${p.attributes.join(", ")} |`,
                )
                .join("\n");
          } else {
    ...
    if (name === "analyze_action") {
      const filePath = args?.filePath as string;
      const methodName = args?.methodName as string;
      const analysis = await analyzeActionRelations(
        PROJECT_ROOT,
        filePath,
        methodName,
      );
