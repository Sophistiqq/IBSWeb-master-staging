...
export async function getDbPool(projectRoot: string) {
  ...
  if (!fs.existsSync(appSettingsPath)) {
    throw new Error(
      `Could not find appsettings.Development.json at ${appSettingsPath}`,
    );
  }
  ...
  if (!connectionString) {
    throw new Error(
      "DefaultConnection string not found in appsettings.Development.json",
    );
  }

  // Convert .NET connection string to pg format if necessary
  // Standard Npgsql string: "Server=localhost;Port=5432;Database=ibs_dev;User Id=postgres;Password=mis123"
